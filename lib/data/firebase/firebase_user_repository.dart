...
class FirebaseUserRepository implements UserRepository {
  ...
  Future<Result<User>> uploadProfilePicture(
    ...
    Reference reference =
        FirebaseStorage.instance.ref().child('PP_OF_${user.uid}');
  ...
  @override
  Future<Result<User>> deleteProfilePicture({required User user}) async {
    Reference reference = FirebaseStorage.instance.ref();
    try {
      await reference.child('PP_OF_${user.uid}').delete();
      var updateResult = await updateUser(user: user.copyWith(photoUrl: null));
      if (updateResult.isSuccess) {
        return Result.success(updateResult.resultValue!);
      } else {
        return Result.failed(updateResult.errorMessage!);
      ...
      return const Result.failed('Failed to delete profile picture');
    }
  }
  ...
  Future<Result<User>> updateUserName(
    ...
    try {
      var result = await updateUser(user: user.copyWith(name: name));
      if (result.isSuccess) {
      ...
      return const Result.failed("Failed to update profile");
    }
  }
}
